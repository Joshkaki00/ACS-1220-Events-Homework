...
def index():
    ...
    events = Event.query.order_by(Event.date_and_time).all()
...
def create():
    ...
    if request.method == 'POST':
        ...
        return render_template('create.html', event_types=EventType)
    ...
    if is_returning_guest:
        ...
        if not guest:
            return render_template('event_detail.html', 
                event=event, 
                error='Guest not found. Please register as a new guest.')
...
def guest_detail(guest_id):
    """Show details for a single guest."""
